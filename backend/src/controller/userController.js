...
const userController = {
  ...
  updateAvatarUser: async (req, res) => {
    try {
      ...
      const avatarPath = await uploadImage(image);
      console.log("🚀 ~ updateAvatarUser: ~ avatarPath:", avatarPath);
      await User.updateOne({ _id: req.user._id }, { avatar: avatarPath });
      return res
        .status(200)
  ...
  updateCoverImageUser: async (req, res) => {
    try {
      const image = req.body.coverphoto;
      const coverPhotoPath = await uploadImage(image);
      console.log(
        "🚀 ~ updateCoverImageUser: ~ coverPhotoPath:",
        coverPhotoPath
      );
      ...
      return res
        .status(200)
        .json({ success: true, message: "Cover photo updated successfully" });
    } catch (error) {
      return res.status(500).json({ success: false, message: error });
    }
  },
