...
const SubmitCommentStyled = styled.div`
  .wrap-input {
    width: 100%;
  }
`;
...
const SubmitComment = () => {
  const { avatar } = MyInfo();
  return (
    <SubmitCommentStyled className="flex items-center md:gap-[18px] gap-[14px] md:mt-10 pb-[30px]">
      <img
        src={avatar}
        alt="avatar"
      ...
      <InputSecondary
        ...
        classNameWrap="text-white"
        className="md:pl-5 md:py-[9px] rounded-[6px] pl-[10px] h-10"
      ></InputSecondary>
      <Button className="md:px-[32px] md:py-[9px] md:w-fit !text-bodyBold font-medium text-white hidden md:block">
        Comment
      </Button>
      <Button className="md:hidden block !w-10 h-10 p-3 flex-shrink-0 bg-grayColor rounded-[5px]">
        <SendIcon />
      </Button>
    </SubmitCommentStyled>
  );
